...
| L-013 | **High** | ops | **The Vercel project serves v0, not v1.** Root Directory is `.`, framework preset "Other", so `lituk-drill.vercel.app` returns the repo-root `index.html` — a second live copy of v0 at a **new origin**. Browser storage is per-origin, so this app looks identical and has an empty schedule: opening it invites either believing six weeks of progress is lost, or splitting study across two stores. `vercel.json` cannot fix it — the Next.js builder resolves `package.json` from the Root Directory and says so in the failure. | 2026-08-04 | 2026-08-07 | `open` — needs Root Directory set to `v1` in project settings. **Owner action; cannot be done from the repo.** Until then, do not open `lituk-drill.vercel.app` on the phone |
